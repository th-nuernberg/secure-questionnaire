...
const lengthnSubstrings = (sentence, word) => {
    return Object.keys(sentence).map(index => parseInt(index)).flatMap(index =>{
        if(index + word.length < sentence.length){
            return [sentence.substring(index, index + word.length),sentence.substring(index, index + word.length+1),sentence.substring(index, index + word.length-1) ];
        }else{
            return []
        }
    } )
...
const containsSimiliarityList = (sentence, wordList, dist=1) => {
    return wordList.map(word => containsSimiliarity(sentence, word, dist)).reduce( (x,y) => x || y)
}
