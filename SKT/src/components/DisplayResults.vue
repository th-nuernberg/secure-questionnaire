  ...
  <body>
    ...
    <table>
      <tr v-for="(item, index) in tableData" :key="item">
        ...
        <button
          @click="play(index)"
          v-if="index + 1 in this.task_data['audio']"
        >
          Play Audio
        </button>
      </tr>
...
export default {
  ...
  created() {
    ...
    this.tableData = this.tableData.filter(
      (value, index) => (index + 1).toString() in this.task_data
    );
  },
  data() {
    return {
      tableData: [
        [
          ...
          "Erkannte Gegenstände: " +
            this.task_data["1"]["images"]
              .map((entry) => entry["recognized"])
              .reduce((x, y) => x + y),
        ],
        [
          "Aufgabe: 2",
        ...
        ["Aufgabe: 3", "Zeit: " + this.task_data["2"]["time"] + " Sekunden"],
      ],
    };
  },
};
